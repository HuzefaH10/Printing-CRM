...
export default function OpportunitiesPage() {
  ...
  return (
    <div className="flex flex-col h-full max-w-[1600px] mx-auto space-y-6">
      <PageHeader
        title="Opportunities"
        description="Manage your commercial printing pipeline and expected revenue."
        actions={
          <>
            <div className="flex bg-muted p-1 rounded-md border mr-2 shadow-sm">
              <button 
                onClick={() => setViewMode("KANBAN")}
                className={`p-1.5 rounded transition-colors ${viewMode === 'KANBAN' ? 'bg-background shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-card border rounded-xl p-4 shadow-sm flex items-center gap-4 hover:border-primary/30 transition-colors">
          <div className="p-3 bg-blue-500/10 rounded-lg text-blue-500"><DollarSign className="w-5 h-5" /></div>
          <div>
            <p className="text-sm font-medium text-muted-foreground">Pipeline Value</p>
            <h3 className="text-xl font-bold text-foreground">{formatCurrency(totalValue, "USD")}</h3>
          </div>
        </div>
        <div className="bg-card border rounded-xl p-4 shadow-sm flex items-center gap-4 hover:border-primary/30 transition-colors">
          ...
          <div>
            ...
            <h3 className="text-xl font-bold text-foreground">{opportunities.filter(o => o.status === "OPEN").length}</h3>
          </div>
        </div>
        <div className="bg-card border rounded-xl p-4 shadow-sm flex items-center gap-4 hover:border-primary/30 transition-colors">
          ...
          <div>
            ...
            <h3 className="text-xl font-bold text-foreground">{wonThisMonth}</h3>
          </div>
        </div>
      </div>
